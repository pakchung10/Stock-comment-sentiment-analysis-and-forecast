    ...
    for page in range(pages):
        ...
        for url_element, author_element, date in zip(url_element_list, author_element_list, date_list):
            ...
            if last_post_month != post_date.month:
                try:
                    if "dougu" in url_element.get_property("href"):
                        continue
...
def get_post_text(post_home_url, post_author, save_path):
    ...
    if "caifuhao" in post_home_url:
        ...
        # 是访谈
        return
    elif "dougu" in post_home_url:
        # 是斗股
        return
    elif "roadshow" in post_home_url:
        # 是直播
        return
    ...
    # 先爬取一定有的主楼内容（格林美资讯除外，没有情感内容）
    post_content = ""
    if post_author != "格林美资讯":
...
def get_url_main():
    ...
    for stock_url, stock_name in zip(stock_url_list, stock_name_list):
        all_post_url = get_stock_all_post_url(stock_url, stock_name, datetime(2020, 1, 1), datetime(2021, 1, 1))
    ...
    for post_url_file in post_url_file_list:
        with open(post_url_file, "r", encoding="utf-8") as file:
            ...
            for line in lines:
                ...
                author_list.append(line.split("\t")[3].strip())
                title_list.append(line.split("\t")[2].strip())
            # for i in range(len(lines)):
            ...
            for i in range(thread_num - 1):
                ...
                thread = threading.Thread(target=get_post_text_thread,
                                          args=(save_path, start_num, url_list[start_num:end_num],
                                                author_list[start_num:end_num], title_list[start_num:end_num],))
                thread_pool.append(thread)
            thread_pool.append(threading.Thread(target=get_post_text_thread,
                                                args=(
                                                    save_path,
                                                    math.ceil(len(lines) / thread_num) * (thread_num - 1),
                                                    url_list[math.ceil(len(lines) / thread_num) * (thread_num - 1):],
                                                    author_list[math.ceil(len(lines) / thread_num) * (thread_num - 1):],
                                                    title_list[math.ceil(len(lines) / thread_num) * (thread_num - 1):],)))
...
def get_post_text_thread(save_path, count_start, post_url_list, author_list, title_list):
    count = count_start
    # print(post_url_list)
    for post_url, post_author, title in zip(post_url_list, author_list, title_list):
        if "业绩网上说明会" in title or "网上业绩说明会" in title:
            count += 1
            continue
        file_name = str(count) + ".txt"
        if not os.path.exists(save_path):
